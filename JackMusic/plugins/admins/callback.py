...
from JackMusic import YouTube, app
from JackMusic.core.call import Alina
from JackMusic.misc import SUDOERS, db
...
from JackMusic.utils.formatters import seconds_to_min
from JackMusic.utils.inline import close_markup, stream_markup, stream_markup_timer, stream_markup2, stream_markup_timer2, panel_markup_5, track_markup, slider_markup, livestream_markup, playlist_markup, stream_markup, stream_markup_timer, telegram_markup, panel_markup_4, panel_markup_3, panel_markup_2, stream_markup_timer2, stream_markup2, queue_markup, panel_markup_1 
from JackMusic.utils.stream.autoclear import auto_clean
...
async def unban_assistant(_, callback: CallbackQuery):
    ...
    try:
        ...
        await callback.answer("⇜ تمت إزالة النطاق الترددي لحساب المساعد بنجاح♥\n\n⇜ الآن يمكنك تشغيل الأغاني🎻\n\n⇜ تشغيل الأغاني‌‌ : /play + اسم الاغنيه", show_alert=True)
    except Exception as e:
        await callback.answer(f"دورفشل في إلغاء ربط حساب المساعد ليس لدي دور\n\n⇜ أعطني دورًا لإلغاء ربط حساب المساعد ", show_alert=True)
    ...
    if command == "Pause":
        ...
        if int(command) in [1, 3]:
            if (duration_played - duration_to_skip) <= 10:
                ...
                return await CallbackQuery.answer(
                    f"<b>» يستطيع الروبوت تقديم الأغنية لأنها انتهت صلاحيتها\n\nالتشغيل : {bet} خلال دقائق‌‌ {duration} </b>",
                    show_alert=True,
                )
